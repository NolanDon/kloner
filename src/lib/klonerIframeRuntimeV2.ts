  ...
  `;
    ...
    async function queueImageUpload(
        ...
        if (typeof uploader !== "function") {
            console.warn("[klonerIframeRuntimeV2] uploadImageAsset handler missing", {
                localId,
                fileName: file.name,
                fileSize: file.size,
                fileType: file.type,
        ...
        console.log("[klonerIframeRuntimeV2] queueing pending image upload", {
            localId,
            fileName: file.name,
            fileSize: file.size,
            fileType: file.type,
        ...
        try {
            ...
            console.log("[klonerIframeRuntimeV2] image upload resolved", {
                localId,
                url: asset?.url || "",
                path: asset?.path || "",
            });
            if (!asset?.url) {
                console.warn("[klonerIframeRuntimeV2] image upload returned no url", { localId });
                return;
            }
            onUploaded(asset);
            console.log("[klonerIframeRuntimeV2] image injected after upload", {
                localId,
                url: asset.url,
                path: asset.path || "",
            });
        } catch (err) {
            console.warn("[klonerIframeRuntimeV2] image upload failed", { localId }, err);
        } finally {
            pendingImageUploads.delete(localId);
            console.log("[klonerIframeRuntimeV2] cleared pending image upload", { localId });
        }
    }
