...
export async function GET(request: NextRequest) {
  ...
  async function computeStatus(params: { uid: string; allowAutoFinalize: boolean; appIdHint?: string }) {
    ...
    try {
      ...
      // If the client tells us which app is being integrated, persist it for the rest of the flow.
      if (appIdHint) {
        try {
          await setupRef.set({ appId: appIdHint, updatedAt: new Date() }, { merge: true });
        } catch {
          // ignore
        }
      }
            ...
            try {
              ...
              if (res.ok) {
                ...
                if (isActiveStatus(remoteStatus)) {
                  ...
                  // Also update .env.local in the app's Firestore files map.
                  const appId =
                    (typeof setup?.appId === "string" && setup.appId.trim())
                      ? setup.appId.trim()
                      : (appIdHint || null);
