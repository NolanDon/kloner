...
import {
  makeRecoveryCheckoutUrl,
  makeUnsubUrl,
} from "@/app/api/private/email-links";
import {
...
function buildTrialWelcomeHtml(args: {
  name?: string | null;
  dashboardUrl: string;
}) {
                    ...
                    <tr>
                        <td style="font-size:15px;line-height:1.65;">
                            ...
                            <p style="margin:0 0 16px 0;">
                                Your Kloner trial is live. Visit the dashboard and enter a URL in the top field to start building.
                            </p>
                            ...
                            <div style="margin:0 0 16px 0;padding:12px;border:1px solid #e5e7eb;border-radius:10px;background:#f9fafb;">
                                <p style="margin:0;font-weight:700;color:#374151;">
                                    The dashboard is where everything starts.
                                </p>
                            </div>
...
function buildTrialWelcomeText(args: {
  name?: string | null;
  dashboardUrl: string;
}) {
  const safeName = (args.name || "there").trim() || "there";
...
async function sendTrialWelcomeEmail(params: {
  ...
  if (
    ...
    throw new Error(
      ((result as any).error?.message as string) || "Email send failed",
    );
  }
...
async function applyAiCreditTopupFromCheckoutSession(
  ...
  const paymentStatus =
    typeof (session as any).payment_status === "string"
  ...
  await db.runTransaction(async (tx: any) => {
    ...
    const bucket =
      data["credits.aiEdits"] || (data.credits && data.credits.aiEdits) || {};
    ...
    const monthlyLimit =
      typeof monthlyLimitRaw === "number" &&
      Number.isFinite(monthlyLimitRaw) &&
      monthlyLimitRaw >= 0
        ? monthlyLimitRaw
        : null;
...
function getInvoiceSubscriptionId(invoice: Stripe.Invoice): string | null {
  ...
  const parentSub =
    typeof anyInv?.parent?.subscription_details?.subscription === "string"
      ...
      : typeof anyInv?.parent?.subscription_details?.subscription?.id ===
          "string"
...
async function resolveUidForCustomerId(
  customerId: string,
): Promise<string | null> {
...
async function ensureAffiliateLockOnFirstPaid(params: {
  ...
  affiliateRefLocked: string;
  affiliateSourceLocked: string;
} | null> {
  ...
  const affiliateSource =
    resolved.affiliateSource ||
    cleanStr(data?.affiliateSource || "") ||
    "unknown";
 ...
 */
async function findInvoiceIdForPaymentIntent(
  stripe: Stripe,
  piId: string,
): Promise<string> {
  if (!piId) return "";
  try {
    ...
    console.error(
      "[stripe] invoices.search payment_intent failed",
      { piId },
      e,
    );
    return "";
...
async function writeAffiliateLedgerForInvoicePaid(params: {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (!snap.exists) {
      ...
      if (!cur?.paymentIntentId && paymentIntentId)
...
async function handleRefundCreatedForAffiliate(params: {
  ...
  const invoiceId = piId
    ? await findInvoiceIdForPaymentIntent(stripe, piId)
    : "";
  if (invoiceId) {
  ...
  try {
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        const checkoutFlow = cleanStr(
          (session.metadata as any)?.checkoutFlow,
          64,
        );
        const trialWelcomeEmail = cleanStr(
          (session.metadata as any)?.trialWelcomeEmail,
          16,
        );
        const shouldSendTrialWelcome =
          !!firebaseUid &&
          (checkoutFlow === "app_deploy_trial" || trialWelcomeEmail === "1");
      ...
      case "customer.subscription.trial_will_end": {
        ...
        const effectiveTier = effectiveTierFromStripeSubscription({
          ...
          created:
            typeof (sub as any).created === "number"
              ? (sub as any).created
              : null,
        });
      ...
      case "invoice.payment_succeeded": {
        ...
        if (!piId || !chId) {
          try {
            ...
            console.error(
              "[stripe] invoice.retrieve for ids failed",
              { invoiceId: inv.id },
              e,
            );
          }
      ...
      case "charge.refunded": {
        ...
        if (obj?.object === "charge") {
          ...
          await handleRefundCreatedForAffiliate({
            stripe,
            refund: obj as Stripe.Refund,
          });
        }
        break;
      }
